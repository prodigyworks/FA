		...
		if (! isset($_SESSION['SITE_CONFIG'])) {
			...
			if ($result) {
				if (mysql_num_rows($result) == 1) {
					...
					$data->maintenancemode = $member['maintenancemode'];
...
function smtpmailer($to, $from, $from_name, $subject, $body, $attachments = array()) { 
	...
	try {
		...
		$mail->SetFrom("info@hyfl.website", $from_name);
		$mail->IsHTML(true);
...
function sendRoleMessage($role, $subject, $message, $attachments = array()) {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'info@hyfl.website', 'Harrow Youth Football League', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(), $attachments);
...
function sendClubMessage($clubid, $subject, $message, $attachments = array()) {
	$qry = "SELECT B.email, B.firstname 
			FROM {$_SESSION['DB_PREFIX']}teamagegroup b
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'confirmation@hysl.com', 'Harrow Youth Football League', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(), $attachments);
		}
...
function sendInternalRoleMessage($role, $subject, $message, $attachments = array()) {
	$from = "info@hyfl.website";
	$fromName = "Harrow Youth Football League";
...
function sendTeamMessage($id, $subject, $message, $footer = "", $attachments = array()) {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'confirmation@hysl.com', 'Harrow Youth Football League', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer, $attachments);
...
function sendUserMessage($id, $subject, $message, $footer = "", $attachments = array(), $action = "") {
	...
	if($result) {
		while (($member = mysql_fetch_assoc($result))) {
			smtpmailer($member['email'], 'info@hyfl.website', 'Harrow Youth Football League', $subject, getEmailHeader() . "<h4>Dear " . $member['firstname'] . ",</h4><p>" . $message . "</p>" . getEmailFooter(). $footer, $attachments);
...
<script>
	$(document).ready(
			function() {
				...
				$("#<?php echo $id; ?>_lazy").change(
						function() {
							callAjax(
									"finddata.php", 
									{ 
										sql: "SELECT <?php echo $value; ?> FROM <?php echo $table; ?> WHERE <?php echo $name; ?> = '" + $(this).val() + "'"
									},
									function(data) {
										if (data.length == 1) {
											$("#<?php echo $id; ?>").val(data[0].<?php echo $value?>);
...
<?php
	...
	if (! $inputname) {
?>
<input type="hidden" id="<?php echo $id; ?>" name="<?php echo $id; ?>" value="0" />
<?php
...
function getLoggedOnClubName($id = "") {
	start_db();
